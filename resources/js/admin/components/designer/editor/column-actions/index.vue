...
const columnForm = ref<ComponentSchema & { [key: string]: any }>({
    type: 'button',
    label: '按钮',
    componentProps: {
        type: 'primary',
...
const attributes: ComponentSchema[] = [
    {
        componentProps: {
            disabled: true,
        },
...
const conditionForm = ref<ComponentSchema & { [key: string]: any }>({} as ComponentSchema)
const conditionIndex = ref<number | null>(null)
const conditionFormRef = useTemplateRef<FormInst>('conditionFormRef')
const conditionType = ref<'disable' | 'show'>('disable')

const MonacoEditor = pluginManager.getComponent('monacoEditor')

function handleShowCondition(index: number, type: 'disable' | 'show') {
    conditionIndex.value = index
    conditionForm.value = cloneDeep(schema.value?.[index])
    conditionType.value = type
    conditionModal.value = true
}

function handleEditCondition() {
    conditionFormRef.value
        ?.validate()
        .then(() => {
            schema.value[conditionIndex.value!] = conditionForm.value
            conditionModal.value = false
        })
        .catch(() => {})
}
</script>
...
<template>
    <div class="w-full text-xs">
                ...
                <template #item="{ element, index }">
                    ...
                    >
                        ...
                        <div class="flex h-full items-center gap-2">
                            <div
                                class="aspect-square h-full cursor-pointer overflow-hidden"
                                @click="handleShowCondition(index, 'disable')"
                            >
                                <Icon name="lsicon:disable-outline" />
                            </div>
                            <div
                                class="aspect-square h-full cursor-pointer overflow-hidden"
                                @click="handleShowCondition(index, 'show')"
                            >
                                <Icon name="mdi:show-outline" />
                            </div>
        ...
        <NModal
            ...
            content-style="max-height: 600px;overflow-y: auto;"
        >
            <NForm ref="conditionFormRef" :model="conditionForm" class="py-4">
                <MonacoEditor
                    v-if="conditionType === 'disable'"
                    v-model="conditionForm.disable"
                    auto-toggle-theme
                    language="javascript"
                />
                <MonacoEditor v-else v-model="conditionForm.show" auto-toggle-theme language="javascript" />
            </NForm>
            <template #footer>
                <NSpace>
                    <NButton size="small" type="primary" @click="handleEditCondition"> 确定 </NButton>
                    <NButton size="small" @click="conditionModal = false"> 取消 </NButton>
                </NSpace>
            </template>
        </NModal>
    </div>
